...
async function run() {
  const { data: settings } = await supabase.from('crm_settings').select('id, user_id, global_system_prompt');
  console.log("Settings global system prompt:", settings);

  const { data: userSettings } = await supabase.from('crm_settings').select('*');
  console.log("All settings:", userSettings);
}
