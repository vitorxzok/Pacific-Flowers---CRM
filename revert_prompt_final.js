  ...
  for (const user of data.users) {
    ...
    if (prompt && prompt.includes("ENVIO CATÁLOGO EM 3 SEGUNDOS")) {
      let newPrompt = prompt.replace(
        /ENVIO CATÁLOGO EM 3 SEGUNDOS \(INSTRUÇÃO DE SISTEMA: Chame a ferramenta 'sendAttachment' com gatilho 'CATALOGO' nesta etapa. NUNCA escreva no texto a ação de enviar, o cliente não pode ver comandos de sistema.\)/g,
        "(ENVIAR CATÁLOGO)"
      );
